...
test("COST_MODE=balanced (default): competitor/price turns use premium", () => {
  const prev = process.env.COST_MODE;
  delete process.env.COST_MODE;
  try {
    assert.equal(costMode(), "balanced");
    assert.equal(classifyTurn("Honda se compare karo EMI kam karo", []), "premium");
...
test("COST_MODE=strict: never promotes to gpt-4o", () => {
  const prev = process.env.COST_MODE;
  process.env.COST_MODE = "strict";
  try {
